...
转义字符以`\0``\x`开头，以`\0`开头表示后面跟八进制的编码值，以`\x`开头的表示后面跟着十六进制编码值，

ASCII码中总共收录了128个字符，而`\0`和`\x`后面只能跟两位数字，所以八进制形式并不能将ASCII码中的所有字符都表示出来，而16进制才能将ASCII码中所有的字符表示出来。
...
Hex: 123xyz
```
因为`\0`和`\x`后面只能跟两位数字，因此用八进制表示x、y、z时，八进制数字有三位为170、171、172。所以我们并不能用八进制来表示x、y、z。
